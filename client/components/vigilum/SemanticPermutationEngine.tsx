      ...
      <div className="container mx-auto max-w-7xl">
        ...
        {permutationResult && (
          <div className="mb-12 text-center">
            ...
            >
              <div className="text-sm font-mono text-gray-400 mb-2">MATHEMATICAL FORMULATION</div>
              <div className="text-2xl font-mono text-white mb-4">
                {permutationResult.mathematical_result.formula}
              </div>
              <button
              ...
              >
                {showFullCalculation ? "Hide Calculation" : "View Full Calculation"}
              </button>
            </div>
          </div>
        )}
        ...
        <AnimatePresence>
          {showFullCalculation && permutationResult && (
            <motion.div
            ...
            >
              ...
              >
                <h3 className="text-lg font-bold text-white font-mono mb-6">OPERATOR CALCULATION BREAKDOWN</h3>
                    ...
                    return (
                      <div key={opId} className="space-y-3">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm text-gray-400">({operator.name})</span>
                        </div>
                        <div className="text-sm font-mono text-gray-300 space-y-1">
                          {details.breakdown.map((item: any, idx: number) => (
                            <div key={idx} className="flex justify-between">
                              <span>w{idx + 1} × var{idx + 1}:</span>
                              <span>
                                {item.weight.toFixed(1)} × {item.value.toFixed(2)} = {item.contribution.toFixed(3)}
                              </span>
                            </div>
                          ))}
                          <div className="border-t border-gray-600 pt-1 flex justify-between font-bold">
                            <span>{opId} =</span>
                            <span style={{ color: operator.color }}>{details.value.toFixed(3)}</span>
                          </div>
                        </div>
                      </div>
                    );
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              />
              <h3 className="text-xl font-bold text-white font-mono">OVERRIDE RESOLUTION PATH</h3>
            </div>
            ...
            {OVERRIDE_MATRIX.map((override, index) => {
              ...
              return (
                <motion.div
                  ...
                  animate={{
                    boxShadow: isActive ? `0 0 20px rgba(239, 68, 68, 0.3)` : "none"
                  }}
                >
                  <div className="flex items-center space-x-4">
                    {/* From Node */}
                    <div
                      className={`text-sm font-mono font-bold px-3 py-2 rounded border-2 ${
                        isActive ? "border-white" : "border-opacity-50"
                      }`}
                      style={{
                        backgroundColor: `${fromOp.color}30`,
                        color: fromOp.color,
                        borderColor: isActive ? fromOp.color : `${fromOp.color}50`,
                      }}
                    >
                      {override.from}
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <div 
                        className={`w-8 h-0.5 ${isActive ? "bg-red-400" : "bg-gray-500"}`}
                      />
                      <div 
                        className={`w-0 h-0 border-l-4 border-t-2 border-b-2 border-t-transparent border-b-transparent ${
                          isActive ? "border-l-red-400" : "border-l-gray-500"
                        }`}
                      />
                    </div>
                    ...
                    <div
                      ...
                      style={{
                        ...
                        color: toOp.color,
                        borderColor: isActive ? toOp.color : `${toOp.color}50`,
                      }}
                    >
                      {override.to}
                    </div>
                  ...
                  <div className="flex items-center space-x-4 flex-1 justify-end">
                    <div className="text-xs text-gray-300 max-w-md text-right font-mono">
                      {override.effect}
                    </div>
